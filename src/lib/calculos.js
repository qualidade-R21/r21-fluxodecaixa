...
export function calcAporteTotalNecessario(contasAPagar, saldoAtual, margemAporteTotal = 0) {
  if (contasAPagar > saldoAtual) {
    return contasAPagar - saldoAtual + (margemAporteTotal || 0);
  }
  return 0;
}
...
export function calcAportesPorSemana(lancamentos, empreendimento, saldoEmp, semanasOrdenadas, equalizacaoComFator, despesasProjetosPorSemana = {}, projetosInternos = [], saldosAcumulados = {}) {
  ...
  for (let i = 0; i < semanasOrdenadas.length; i++) {
    ...
    if (i === 0) {
      ...
      // Semanas subsequentes: sem condicional SE — permite valores negativos (excedente)
      aporteSemana = despesasSemana - saldoEfetivo + (empreendimento.margem_seguranca_demais || 0);
    }
